...
export const MonitoringView = ({ assignedModels }: MonitoringViewProps) => (
    <div className="space-y-8">
        <div className="flex items-center justify-between">
            <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Line Efficiency Monitoring</h2>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {assignedModels.length > 0 ? Array.from(new Set(assignedModels.map(m => m.line_name))).filter(Boolean).map((lineName) => (
                <div key={lineName} className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm group hover:border-[#F37021]/30 transition-all duration-500">
                    <div className="flex justify-between items-start mb-10">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight">{lineName}</h3>
                        ...
                        <div className="text-right">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Current Efficiency</span>
                    ...
                    <div className="grid grid-cols-2 gap-6 pt-10 border-t border-slate-50">
                        <div className="bg-slate-50/50 rounded-2xl p-6">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Avg. Takt Time</span>
                            <div className="flex items-end gap-2">
                                <span className="text-2xl font-black text-slate-900 tracking-tighter tabular-nums">124s</span>
                                <span className="text-[10px] font-black text-slate-400 uppercase mb-1">/UNIT</span>
                            </div>
                        </div>
                        <div className="bg-slate-50/50 rounded-2xl p-6">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Active DEOs</span>
                            <div className="flex items-end gap-2">
                                ...
                                <span className="text-[10px] font-black text-slate-400 uppercase mb-1">OPERATORS</span>
                            </div>
                        </div>
                    </div>
                </div>
            )) : (
                <div className="col-span-full bg-white rounded-[2rem] p-10 border border-slate-100 shadow-sm text-center">
                    ...
                    <p className="max-w-md mx-auto text-slate-400 font-bold text-xs uppercase tracking-widest leading-relaxed">
...
export const ProgressView = ({ assignedModels }: ProgressViewProps) => (
    <div className="space-y-8">
        <div className="flex items-center justify-between">
            <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Model Completion Tracking</h2>
        </div>
        <div className="grid grid-cols-1 gap-8">
            {assignedModels.length > 0 ? assignedModels.map((model) => (
                <div key={model.id} className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm group hover:border-emerald-500/30 transition-all duration-500 overflow-hidden relative">
                    <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-10">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight">{model.name}</h3>
                                ...
                                <div className="flex items-center gap-4 text-xs font-bold text-slate-400 uppercase tracking-widest">
                                    ...
                                    <span className="text-[#F37021]">Line: {model.line_name || 'N/A'}</span>
                                </div>
                            </div>
                        </div>
                                ...
                                return (
                                    <>
                                        <div className="flex justify-between items-end mb-4">
                                            ...
                                            <span className="text-2xl font-black text-slate-900 tracking-tighter tabular-nums">{Math.round(progress)}%</span>
                                        </div>
                        ...
                        <div className="flex items-center gap-12">
                            <div className="text-center">
                                ...
                                <span className="text-3xl font-black text-slate-900 tracking-tighter">{model.planned_qty || 0}</span>
                            </div>
                            <div className="text-right">
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Actual</span>
                                <span className="text-3xl font-black text-emerald-500 tracking-tighter">{model.actual_qty || 0}</span>
                            </div>
                        </div>
                    </div>
                    {/* Abstract background accent */}
                    <div className="absolute top-0 right-0 w-64 h-64 bg-slate-50/50 rounded-full -translate-y-32 translate-x-32 blur-3xl pointer-events-none" />
                </div>
            )) : (
...
export const ReportsView = () => (
    <div className="space-y-8">
        <div className="flex items-center justify-between">
            <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Shift Data Reports</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
                ...
                <div key={i} className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-sm hover:border-[#F37021]/20 transition-all group">
                    <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 mb-6 group-hover:bg-[#F37021]/10 group-hover:text-[#F37021] transition-colors">
                        <report.icon size={24} />
                    </div>
                    <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight mb-1">{report.title}</h3>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">{report.date}</p>
                    <div className="flex items-center justify-between pt-6 border-t border-slate-50">
                        <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest px-3 py-1 bg-slate-50 rounded-full">{report.status}</span>
                        <button className="text-[10px] font-black text-[#F37021] uppercase tracking-widest hover:translate-x-1 transition-transform">Download →</button>
                    </div>
                </div>
            ))}
        </div>
...
export const AlertsView = () => (
    <div className="space-y-8">
        <div className="flex items-center justify-between">
            <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Critical Notifications</h2>
        </div>
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            {[
                ...
                <div key={i} className="p-8 border-b border-slate-50 last:border-0 flex items-start gap-6 hover:bg-slate-50/50 transition-colors">
                    <div className={cn(
                    ...
                    <div className="flex-1">
                        <div className="flex justify-between items-start mb-1">
                            <h3 className="text-base font-black text-slate-900 uppercase tracking-tight">{alert.title}</h3>
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{alert.time}</span>
                        </div>
                        <p className="text-xs font-bold text-slate-500 tracking-tight">{alert.desc}</p>
                    </div>
                </div>
            ))}
        </div>
