...
export const DEOSubmissionHistory: React.FC<DEOSubmissionHistoryProps> = ({
    ...
    if (isLoadingHistory) {
        return (
            <div className="flex flex-col items-center justify-center py-32 space-y-6">
                <div className="w-16 h-16 border-4 border-slate-100 border-t-[#F37021] rounded-full animate-spin" />
    ...
    if (selectedHistoryLog) {
        return (
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <span className="text-[10px] font-black text-[#F37021] uppercase tracking-[0.2em] mb-1 block">Submission Report</span>
                            <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tight">{selectedHistoryLog.model_name}</h2>
                                    ...
                                    <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="px-6 py-5 border-r border-slate-50">
                                            <div className="flex flex-col gap-1">
                                                <span className="text-[11px] font-black text-slate-900 uppercase tracking-tight">{row["PART NUMBER"] || 'NO PART #'}</span>
                                                <span className="text-[9px] font-bold text-slate-400 uppercase tabular-nums">{row["SAP PART NUMBER"] || row["SAP PART #"] || 'NO SAP #'}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-6 py-5 text-center">
                                            {row.row_status === 'VERIFIED' ? (
                                                ...
                                                <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest">Pending Review</span>
                                            )}
                                        </td>
                                    </tr>
                                ))}
    ...
    return (
        <div className="space-y-8">
            <div className="bg-white rounded-[3rem] p-10 border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                <div className="flex items-center gap-6">
                    <div className="w-16 h-16 bg-slate-900 rounded-2xl flex items-center justify-center text-white shadow-xl">
                        <Database size={32} />
                    </div>
                    <div>
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1 block">Log Registry</span>
                        <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tight">Submission History</h2>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">
            ...
            {submissionHistory.length === 0 ? (
                <div className="bg-white rounded-[3.5rem] p-24 border border-slate-100 shadow-sm text-center">
                    <Database size={48} className="text-slate-100 mx-auto mb-8" />
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">No submission history found</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {submissionHistory.map((log) => (
                        <div 
                            ...
                            className="bg-white rounded-[3rem] p-8 border border-slate-100 shadow-sm hover:shadow-xl hover:border-[#F37021]/20 transition-all cursor-pointer group"
                        >
                            <div className="flex justify-between items-start mb-10">
                                <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-orange-50 group-hover:text-[#F37021] transition-colors">
                                    <Clock size={24} />
                                </div>
                            ...
                            <h3 className="text-xl font-black text-slate-900 uppercase tracking-tight mb-2">{log.model_name}</h3>
