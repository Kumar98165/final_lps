    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            {slicers.map((slicer, i) => (
                <div key={i} className="bg-white border border-slate-100 rounded-[2rem] p-5 shadow-sm hover:shadow-md transition-shadow group cursor-pointer">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="p-2 bg-slate-50 text-slate-400 rounded-xl group-hover:bg-slate-100 group-hover:text-slate-900 transition-colors">
                            <slicer.icon size={14} />
                        </div>
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{slicer.label}</span>
                    </div>
                    <div className="flex items-center justify-between px-1">
                        <span className="text-xs font-black text-slate-900 uppercase tracking-tight">{slicer.placeholder}</span>
