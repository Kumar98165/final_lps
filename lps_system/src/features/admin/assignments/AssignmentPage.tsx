                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-4">
                        ...
                        <div className="flex items-center gap-4 w-full md:w-auto">
                            <div className="relative group w-full md:w-96">
                                ...
                                <input
                                    type="text"
                                    placeholder="Find models by name or Reference ID..."
                                    value={searchTerm}
                                ...
                                >
                                    ...
                                    <div className={`rounded-2xl p-4 grid grid-cols-1 gap-3 mb-2 transition-colors relative ${
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-xl shrink-0 ${model.line_id ? (isCompleted ? 'bg-white/60 text-emerald-600 shadow-sm border border-emerald-100/50' : 'bg-white text-orange-600 shadow-sm border border-orange-100') : 'bg-white/30 text-slate-300'}`}>
                                                <Hash size={15} strokeWidth={2.5} />
                                            </div>
                                            <div className="min-w-0">
                                                <p className={`text-[8px] font-black uppercase tracking-[0.1em] mb-0.5 ${isCompleted ? 'text-emerald-600/60' : 'text-slate-400'}`}>Line</p>
                                                <p className={`text-xs font-black truncate leading-tight ${model.line_id ? (isCompleted ? 'text-emerald-900' : 'text-slate-800') : 'text-slate-300 italic'}`}>
                                                    {model.line_name || 'No Line Assigned'}
                                                </p>
                                            </div>
                                        </div>

                                        <div className={`h-px w-full ${isCompleted ? 'bg-emerald-500/10' : 'bg-slate-200/50'}`} />

                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-xl shrink-0 ${model.supervisor_id ? (isCompleted ? 'bg-white/60 text-emerald-600 shadow-sm border border-emerald-100/50' : 'bg-white text-blue-600 shadow-sm border border-blue-100') : 'bg-white/30 text-slate-300'}`}>
                                                <Shield size={15} strokeWidth={2.5} />
                                            </div>
                                            <div className="min-w-0">
                                                <p className={`text-[8px] font-black uppercase tracking-[0.1em] mb-0.5 ${isCompleted ? 'text-emerald-600/60' : 'text-slate-400'}`}>Supervisor</p>
                                                <p className={`text-xs font-black truncate leading-tight ${model.supervisor_id ? (isCompleted ? 'text-emerald-900' : 'text-slate-800') : 'text-slate-300 italic'}`}>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2 rounded-xl shrink-0 ${model.assigned_deo_id ? (isCompleted ? 'bg-white/60 text-emerald-600 shadow-sm border border-emerald-100/50' : 'bg-white text-emerald-600 shadow-sm border border-emerald-100') : 'bg-white/30 text-slate-300'}`}>
                                                <UserIcon size={15} strokeWidth={2.5} />
                                            </div>
                                            <div className="min-w-0">
                                                <p className={`text-[8px] font-black uppercase tracking-[0.1em] mb-0.5 ${isCompleted ? 'text-emerald-600/60' : 'text-slate-400'}`}>DEO</p>
                                                <p className={`text-xs font-black truncate leading-tight ${model.assigned_deo_id ? (isCompleted ? 'text-emerald-900' : 'text-slate-800') : 'text-slate-300 italic'}`}>
                                    ...
                                    <div className="mt-auto flex justify-end pt-1">
                                        <span className={`text-[9px] font-black uppercase tracking-widest flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity translate-x-1 group-hover:translate-x-0 ${isCompleted ? 'text-emerald-600' : 'text-[#F37021]'}`}>
                        ...
                        {filteredModels.length === 0 && (
                            <div className="col-span-full py-20 text-center flex flex-col items-center">
                                ...
                                <p className="text-sm font-medium text-slate-500 mt-1 max-w-sm">
                                    We couldn't find any car models matching "{searchTerm}". Please try a different search term.
                                </p>
                                <button
                    ...
                    >
                            ...
                            {showSuccess && (
                                ...
                                >
                                    ...
                                    <motion.p
                                        ...
                                        className="text-sm font-bold text-slate-500 mt-2"
                                    >
                        ...
                        <div className="relative px-10 pt-16 pb-10 overflow-hidden bg-slate-50/30 shrink-0 border-b border-slate-100/50">
                            ...
                            <div className="relative flex justify-between items-start">
                                <div className="space-y-4">
                                    <div className="inline-flex items-center justify-center w-16 h-16 rounded-[1.5rem] bg-gradient-to-br from-[#F37021] to-[#ff8c42] text-white shadow-[0_12px_24px_-8px_rgba(243,112,33,0.5)] transform hover:rotate-3 transition-transform duration-500">
                                ...
                                <button
                                    onClick={handleCloseModal}
                                    className="p-3 text-slate-400 hover:text-slate-600 hover:bg-white hover:shadow-md hover:scale-110 rounded-2xl transition-all border border-transparent hover:border-slate-100 group"
                                >
                        ...
                        <div className="px-8 py-8 space-y-6 bg-white relative overflow-y-auto flex-1 custom-scrollbar">
                            <div className="grid grid-cols-1 gap-6">
                                ...
                                <div className="space-y-3">
                                    <label className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.15em] text-slate-400 px-1">
                                    ...
                                    <div className="relative group">
                                        <input
                                            type="text"
                                            className="w-full bg-slate-50/50 border border-slate-200 text-slate-700 text-sm font-bold rounded-2xl focus:bg-white focus:ring-[6px] focus:ring-orange-500/5 focus:border-orange-500 block px-5 py-4 outline-none transition-all shadow-sm hover:border-slate-300 group-hover:shadow-md"
                                            value={tempName}
                            ...
                            <div className="flex items-center gap-4 py-2">
                                ...
                                <span className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em] whitespace-nowrap">Assignments</span>
                                <div className="h-px flex-1 bg-gradient-to-r from-transparent via-slate-100 to-transparent" />
                            </div>
                            ...
                            <div className="space-y-3">
                                <label className="flex items-center justify-between px-1">
                                    <span className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.15em] text-slate-400">
                                        <LayoutGrid size={13} className="text-[#F37021]" />
                                        ASSIGN PRODUCTION LINE
                                    </span>
                                </label>
                                <div className="relative group">
                                    <select
                                        className="w-full bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-2xl focus:ring-[6px] focus:ring-orange-500/5 focus:border-orange-500 block px-5 py-4 outline-none transition-all cursor-pointer appearance-none shadow-sm hover:border-slate-300 group-hover:shadow-md"
                                        value={tempLineId}
                                        onChange={(e) => setTempLineId(e.target.value ? Number(e.target.value) : '')}
                                    >
                                        <option value="" className="text-slate-400">-- Select Line --</option>
                                        {lines.map(line => (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="relative group">
                                        <select
                                            className="w-full bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-2xl focus:ring-[6px] focus:ring-blue-500/5 focus:border-blue-500 block px-5 py-4 outline-none transition-all cursor-pointer appearance-none shadow-sm hover:border-slate-300 group-hover:shadow-md"
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="relative group">
                                        <select
                                            className="w-full bg-white border border-slate-200 text-slate-700 text-sm font-bold rounded-2xl focus:ring-[6px] focus:ring-emerald-500/5 focus:border-emerald-500 block px-5 py-4 outline-none transition-all cursor-pointer appearance-none shadow-sm hover:border-slate-300 group-hover:shadow-md"
                                            value={tempDeoId}
                                            onChange={(e) => setTempDeoId(e.target.value ? Number(e.target.value) : '')}
                                        >
                                            <option value="" className="text-slate-400">-- Select DEO --</option>
                                            {deos.map(deo => (
                        ...
                        <div className="px-8 py-6 bg-slate-50/80 backdrop-blur-xl border-t border-slate-100 flex flex-col-reverse sm:flex-row justify-end gap-3 shrink-0">
                            <button
                                onClick={handleCloseModal}
                                disabled={saving}
                                className="px-8 py-4 rounded-2xl font-black text-[11px] uppercase tracking-widest text-slate-400 hover:text-slate-700 hover:bg-white hover:shadow-sm transition-all active:scale-95 disabled:opacity-50"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                disabled={saving}
                                className="group relative flex items-center justify-center gap-3 px-10 py-4 rounded-2xl font-black text-[11px] uppercase tracking-[0.15em] text-white bg-slate-900 border border-slate-900 hover:bg-black hover:border-black shadow-[0_15px_35px_-12px_rgba(0,0,0,0.4)] transition-all active:scale-[0.97] disabled:opacity-70"
                            >
                                {saving ? (
