...
const RowEditModal: React.FC<RowEditModalProps> = ({
    ...
    const renderInput = (label: string, icon?: any) => (
        <div className="space-y-1.5 group">
            <label className="flex items-center gap-2 text-[10px] font-black text-ind-text3 uppercase tracking-widest pl-1">
            ...
            <input
                ...
                className="w-full bg-ind-bg/50 border border-ind-border focus:border-ind-primary/30 focus:bg-white rounded-xl py-2.5 px-4 text-ind-text font-extrabold text-xs outline-none transition-all placeholder:text-slate-200 shadow-sm"
        ...
        <AnimatePresence>
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                <motion.div
                    ...
                    className="relative w-full max-w-6xl bg-white rounded-[2.5rem] shadow-2xl border border-ind-border/50 overflow-hidden flex flex-col max-h-[90vh]"
                >
                    {/* Header */}
                    <div className="p-8 border-b border-ind-border/50 flex items-center justify-between bg-white sticky top-0 z-10">
                    ...
                    <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-[10px] font-black text-ind-primary uppercase tracking-[0.3em]">Identification</span>
                                <div className="h-px flex-1 bg-ind-border/30" />
                            </div>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-3">
                                <div className="h-px flex-1 bg-ind-border/30" />
                                <span className="text-[10px] font-black text-ind-primary uppercase tracking-[0.3em]">Operational Targets & Stock</span>
                                <div className="h-px flex-1 bg-ind-border/30" />
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center gap-3">
                                <div className="h-px flex-1 bg-ind-border/30" />
                                <span className="text-[10px] font-black text-ind-primary uppercase tracking-[0.3em]">Material Data</span>
                                <div className="h-px flex-1 bg-ind-border/30" />
                            </div>
                    ...
                    {/* Footer */}
                    <div className="p-8 bg-ind-bg border-t border-ind-border/50 flex items-center justify-between sticky bottom-0 z-10">
                        <button
                            onClick={() => {
                            ...
                            className="px-6 py-4 flex items-center gap-3 bg-white hover:bg-rose-50 border border-ind-border hover:border-rose-200 text-ind-text3 hover:text-rose-500 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95"
                        >
                            <Trash2 size={16} />
                            Delete Row
                        </button>
                        ...
                        <div className="flex gap-4">
                            <button
                                onClick={onClose}
                                className="px-8 py-4 bg-white hover:bg-ind-border/30 border border-ind-border text-ind-text2 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all active:scale-95"
                            >
                                Cancel
                            </button>
                            <button
                                onClick={() => onSave(formData)}
                                className="px-10 py-4 bg-[#F37021] hover:bg-orange-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl shadow-orange-500/30 active:scale-95 flex items-center gap-2"
                            >
                                <Save size={16} />
                                Save Changes
                            </button>
